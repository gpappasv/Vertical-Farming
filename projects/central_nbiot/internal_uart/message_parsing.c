...
uint8_t process_rx_message(uint8_t *rx_buf)
{
    ...
    case MESSAGE_READY_FOR_CLOUD:
        ...
        break;
    case MESSAGE_CONTROL_GPIOS:
        p_msg_control_gpios = (message_control_gpios_t *)rx_buf;
        // --- calculate crc of the message
        ...
        {
            // TODO: crc error
            // LOG_INF("CRC error, calc crc = %d, received crc = %d", calculated_crc, p_msg_control_gpios->message_crc);
        ...
        else
        {
            ...
            // LOG_INF("Received gpio control msg from 52840");
        }
        // Notify op result reply thread to reply to the message
        k_event_post(&uart_communication_events, MESSAGE_TO_REPLY_EVT);
        break;
    case MESSAGE_UPDATE_TIMESTAMP:
        p_msg_update_timestamp = (message_update_timestamp_t *)rx_buf;
        // --- calculate crc of the message
        ...
        {
            // TODO: crc error
            // LOG_INF("CRC error, calc crc = %d, received crc = %d", calculated_crc, p_msg_update_timestamp->message_crc);
