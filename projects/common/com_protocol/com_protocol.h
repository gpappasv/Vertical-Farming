...
#define MESSAGE_CONTROL_GPIOS 0x82
#define MESSAGE_UPDATE_TIMESTAMP 0x83
...
enum message_operation_resuls_e
{
    ...
    MESSAGE_CONTROL_GPIOS_SUCCESS = 0x100,
    MESSAGE_CONTROL_GPIOS_CRC_ERROR = 0x200,
    UPDATE_TIMESTAMP_MSG_FAIL = 0x300,
    UPDATE_TIMESTAMP_MSG_SUCCESS = 0x400,
    GENERIC_ERROR = 0xFF
};
...
// --- MESSAGE_CONTROL_GPIOS ---
#pragma pack(push, 1)
typedef struct message_control_gpios_s
{
    uint8_t type;
    uint8_t len;
    bool row_fan_control[MAX_CONFIGURATION_ID];
    bool row_water_control[MAX_CONFIGURATION_ID];
    bool row_lights_control[MAX_CONFIGURATION_ID];
    uint16_t message_crc;
} message_control_gpios_t;
#pragma pack(pop)

// --- MESSAGE_UPDATE_TIMESTAMP ---
#pragma pack(push, 1)
typedef struct message_update_timestamp_s
{
    uint8_t type;
    uint8_t len;
    uint16_t message_crc;
} message_update_timestamp_t;
#pragma pack(pop)
