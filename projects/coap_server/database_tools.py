...
class MessageParsing:
    ...
    def insert_into_database(self, id, timestamp, temp, hum, soil, light, lswitch, wswitch, fswitch):
        try:
            ...
            print(f"Can't reconnect to database: {e}")
        ts = int(str(int(timestamp/1000)))
        # Adjust summer time - This could be done through firmware.
        ts += 3600 # summer time with +1 hour
...
class GetDatabaseEntries:
    ...
    def get_most_recent_timestamps(self, list_length) -> list:
        ...
        if list_length > 0:
            ...
            fetch_timestamps_query = "SELECT DISTINCT timestamp FROM row_mean_values ORDER BY timestamp DESC"
        data_list = []
        try:
            connection.reconnect(attempts=2, delay=1)
        except database.Error as e:
        ...
        try:
            ...
            print(f"Error getting timestamps from database: {e}")
            connection.close()
        return data_list
...
class UserRequestsDBTools:
    ...
    def clear_measurements_diagrams(self):
        request_query_measurements = "DELETE from row_mean_values"
        request_query_control = "DELETE from user_control_request"
        request_query_thresholds = "DELETE from user_thresholds_request"
        try:
            connection.reconnect(attempts=2, delay=1)
        except database.Error as e:
            print(f"Can't reconnect to database: {e}")
        try:
            cursor.execute(request_query_measurements)
            connection.commit()
            cursor.execute(request_query_control)
            connection.commit()
            cursor.execute(request_query_thresholds)
            connection.commit()
            connection.close()
        except database.Error as e:
            print(f"Error adding entry to database: {e}")
